...
// Pure: assemble the per-season projection inputs from each season's fresh
// catalog bucket and its prior scan:data (index-aligned with `seasons`).
//
// Pid sets are the UNION of both so same-season consignment-only/archived
// products survive. SKUs, however, are overlaid catalog-LAST across ALL seasons:
// the catalog snapshot is fresher, and a per-season merge would let one season's
// stale scan:data clobber another season's corrected SKU. Without this, a SKU
// re-coded in Lightspeed (e.g. "/f26" -> "/ps27") keeps projecting its PO into
// the old season, so the same order is counted under two seasons.
export function seasonProjectionInputs(seasons = [], { catalogBuckets = [], priorData = [] } = {}) {
  const seasonPidSets = {};
  const scanRanges = {};
  const priorPidToSku = {};
  const catalogPidToSku = {};

  seasons.forEach((season, i) => {
    const bucket = catalogBuckets[i];
    const prior = priorData[i];
    seasonPidSets[season] = new Set([...(bucket?.seasonPids || []), ...(prior?.seasonPids || [])]);
    scanRanges[season] = seasonScanDateRange(season);
    Object.assign(priorPidToSku, prior?.pidToSku || {});
    Object.assign(catalogPidToSku, bucket?.pidToSku || {});
  });

  return { seasonPidSets, scanRanges, pidToSku: { ...priorPidToSku, ...catalogPidToSku } };
}
