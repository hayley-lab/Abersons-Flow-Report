...
// is already fully built in KV. A sync cycle explicitly requests one incremental
// refresh; after that refresh drains, later calls must reserve their budget for
// season steps. Re-driving every complete cache on every call deterministically
// starves season work when the four drives consume the step-headroom budget.
// Explicit cold resets and a pending cycle refresh always drive.
export function shouldSkipCompletedDrive({
  kvComplete = false,
  resetRequested = false,
  refreshRequested = false,
}) {
  return !!kvComplete && !resetRequested && !refreshRequested;
}
...
// Decide whether a FAILED season step should be retried on a later pass
// (leave done:false) rather than abandoned (done:true).
//   - The internally-looping Vercel cron path already retries within its own
//     loop, so it always keeps the season pending (unchanged behavior).
//   - The GitHub/UI driver path makes one bounded pass per HTTP call. A single
//     transient failure (network/timeout/5xx) should NOT fail the whole weekly
//     sync — retry it up to `maxAttempts` (across passes/calls) before giving
//     up. A terminal failure (the step returned phase:"error", or a 4xx) is
//     never retried here; it surfaces to the workflow's error check.
export function shouldRetrySeasonStep({ loopInternally, transient, attempts, maxAttempts = 3 }) {
...
// per-step results collected this pass, plus every season not stepped this call.
// Pending work must never be mislabeled "skipped" — workflows use this action to
// distinguish real progress from cache-only calls and detect stalls.
export function buildDriverResults(seasonState, stepResults) {
  const results = [...stepResults];
  const represented = new Set(stepResults.map((result) => result.season));
  for (const s of seasonState) {
    if (represented.has(s.season)) continue;
    if (s.done && s.action === "skipped")
      results.push({ season: s.season, action: "skipped", phase: s.phase, mode: s.mode });
    else
      results.push({
        season: s.season,
        action: "pending",
