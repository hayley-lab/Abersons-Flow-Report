...
export function assertKvValueSize(key, value, maxBytes = DEFAULT_KV_MAX_VALUE_BYTES) {
  ...
  if (size > maxBytes) {
    throw new Error(
      `KV value "${key}" is ${size} bytes, above the safe ${maxBytes} byte limit`
    );
  }
  return size;
}
...
export async function delShardedObject(kv, baseKey, { shardCount = DEFAULT_KV_SHARD_COUNT } = {}) {
  ...
  const effectiveShardCount = isShardedKvObject(marker) ? marker.shardCount || shardCount : shardCount;
  await Promise.all([
    kv.del(baseKey),
