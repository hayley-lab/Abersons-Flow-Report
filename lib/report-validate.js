...
//   - Fields whose value comes from the RMH/datatail override overlay (returns,
//     and closed-season sold/on-sale) are NOT LS-verifiable either: that data
//     never existed in Lightspeed, so the LS-only re-derivation legitimately
//     can't reproduce it. Such fields are reported as skipped (override-sourced),
//     never failures. The row's other fields (e.g. live on-hand, LS POs) are
//     still checked.
...
export const SKIP_REASONS = {
  ...
  OVERRIDE_SOURCED: "override-sourced",
};

// Hard quantity fields: a single exact mismatch on any of these on a verifiable
// row trips the drift threshold on its own (no tolerance). These are re-projected
// from the same LS consignment cache the rows were built from, so the comparison
// is deterministic and any mismatch is a genuine logic bug.
//
// onHand is deliberately NOT hard. It compares a scan-time snapshot against a
// live LS fetch made hours later, so an item that sells or arrives overnight is a
// legitimate difference, not drift — as a hard trip it failed the nightly Action
// on normal trading (fall26: 2 single-unit moves out of 4011 rows). It is still
// recorded, and still counts toward the drifted-product ratio and the season
// dollar-drift signal, so a SYSTEMATIC on-hand fault trips those thresholds.
// Sales sold/onSale qty drift is likewise a signal rather than an automatic trip.
const HARD_QTY_FIELDS = new Set(["qtyOrdered", "qtyReceived", "retQty"]);
...
export function buildValidationReport({
  ...
  for (const row of rows || []) {
    ...
    // A field is override-sourced (RMH/datatail overlay) when buildAllRows
    // tagged it on the row. Such values never existed in Lightspeed, so the
    // LS-only re-derivation can't reproduce them -> skip, never fail.
    const overrideSet = Array.isArray(row.overrideFields) ? new Set(row.overrideFields) : null;
    const isOverride = (field) => !!overrideSet && overrideSet.has(field);
    function skipOverride(field) {
      overrideSourcedFields += 1;
      skipped.push({ pid: row.pid, sku, field, reason: SKIP_REASONS.OVERRIDE_SOURCED });
    }
    ...
    if (isOverride("sold")) {
      skipOverride("sold");
    } else if (num(fs.sold) !== num(row.sold)) {
      record(row, "sold", num(row.sold), num(fs.sold), "sales", false);
      seasonRetailDrift += Math.abs(num(fs.sold) - num(row.sold)) * price;
    }
    if (isOverride("onSale")) {
      skipOverride("onSale");
    } else if (num(fs.onSale) !== num(row.onSale)) {
      record(row, "onSale", num(row.onSale), num(fs.onSale), "sales", false);
    }
    if (isOverride("saleAmt")) {
      skipOverride("saleAmt");
    } else if (!withinDollarTolerance(row.saleAmt, fs.saleAmt, row.onSale, t)) {
