...
// marker carrying the advanced cursor), so a larger interval means fewer, still
// cheap, KV writes per chunk. The old code full-rewrote all 16 store shards
// after EVERY page (~220 pages on a cold build), which blew the function
// timeout; periodic touched-shard checkpoints keep each save O(delta).
const DEFAULT_CHECKPOINT_EVERY_PAGES = 20;
