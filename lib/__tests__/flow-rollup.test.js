...
describe("rollup — vendor-level ordered combine rule", () => {
  ...
  it("keeps datatail ordered when the LS pid has ONLY a vendor return (no PO)", () => {
    ...
    const { deptVendors } = rollup(buildAllRows(scanData, override), scanData, override);
    // Ordered stays (not dropped by the return-as-overlap bug). Product qty is
    // gross, so Ordered $ nets like old flow.sql: (5 − 2) × $100 = $300.
    expect(vendorRow(deptVendors, "alley", "ReturnOnly").ordered).toBe(300);
  });

  it("nets datatail-only ordered dollars against vendor returns (Hayley)", () => {
    const scanData = makeScanData({
      seasonPids: ["ls"],
      productStats: {
        ls: { qtyOrdered: 6, qtyReceived: 6, retQty: 0, sold: 0, onSale: 0, liveOnHand: 6 },
      ...
      pidToType: { ls: "alley" },
      pidToSupplier: { ls: { id: "M", name: "Mixed Vendor" } },
      pidToPrice: { ls: 100 },
      pidToSku: { ls: "mix/s2601" },
      skuToPid: { "mix/s2601": "ls" },
    });
    const override = {
      stores: {},
      vendors: {
        mixed: {
          ...
          ordered: 700,
          products: [
            { style: "mix/s2601", price: 100, qtyOrdered: 6 }, // overlaps LS, ignored
            { style: "mix/s2603", price: 100, qtyOrdered: 5, qtyReturned: 2 }, // unique, net
          ],
        },
      },
    };
    const { deptVendors } = rollup(buildAllRows(scanData, override), scanData, override);
    const v = vendorRow(deptVendors, "alley", "Mixed Vendor");
    expect(v.ordered).toBe(900); // LS 600 + unique (5 − 2) × $100
  });

  it("nets RMH-backfill gross vendor ordered using orderedScraped", () => {
    const scanData = makeScanData(); // no LS pids — vendor-level fallback
    const override = {
      stores: {},
      vendors: {
        v: {
          ...
          ordered: 500,
          orderedScraped: 300,
          products: [{ style: "old/s2601", price: 100, qtyReturned: 2 }],
        },
      },
    };
    const { deptVendors } = rollup(buildAllRows(scanData, override), scanData, override);
    expect(vendorRow(deptVendors, "alley", "Old Brand").ordered).toBe(300);
  });

  it("does not subtract returns from already-net scraped vendor ordered", () => {
    const scanData = makeScanData();
    const override = {
      stores: {},
      vendors: {
  ...
  it("nets rmhret__ returns onto ordered SKUs in the same vendor bucket (Staud)", () => {
    // Production spring26 Staud: ordered lives on 5__942, returns on rmhret__5__942.
    const scanData = makeScanData({
      seasonPids: ["p1"],
      productStats: {
        p1: { qtyOrdered: 0, qtyReceived: 0, retQty: 0, sold: 0, onSale: 0, liveOnHand: 3 },
      },
      pidToType: { p1: "alley" },
      pidToSupplier: { p1: { id: "942", name: "Staud" } },
      pidToPrice: { p1: 100 },
      pidToSku: { p1: "staud/s2601" },
      skuToPid: { "staud/s2601": "p1" },
    });
    const override = {
      stores: {},
      vendors: {
        "5__942": {
          ...
          deptName: "Alley",
          ordered: 500,
          products: [{ style: "staud/s2601", price: 100, qtyOrdered: 5, qtyReturned: 0 }],
        },
        rmhret__5__942: {
          vendorId: "942",
          vendorName: "Staud",
          ...
          products: [{ style: "staud/s2601", price: 100, qtyOrdered: 0, qtyReturned: 2 }],
        },
      },
    };
    const rows = buildAllRows(scanData, override, { season: "spring26" });
    const { deptVendors } = rollup(rows, scanData, override, { season: "spring26" });
    const v = vendorRow(deptVendors, "alley", "Staud");
    expect(v.ordered).toBe(300); // (5 − 2) × $100
    expect(v.returned).toBe(200);
  });

  it("keeps LS-overlap SKUs LS-only when rmhret__ carries the same SKU", () => {
    const scanData = makeScanData({
      seasonPids: ["p1"],
      productStats: {
        p1: { qtyOrdered: 5, qtyReceived: 5, retQty: 2, sold: 0, onSale: 0, liveOnHand: 3 },
      },
      pidToType: { p1: "alley" },
      pidToSupplier: { p1: { id: "942", name: "Staud" } },
      pidToPrice: { p1: 100 },
    ...
    const override = {
      ...
      vendors: {
        "5__942": {
          ...
          deptName: "Alley",
          ordered: 500,
          products: [{ style: "staud/s2601", price: 100, qtyOrdered: 5 }],
        },
        rmhret__5__942: {
          vendorId: "942",
          vendorName: "Staud",
          deptName: "Alley",
          ordered: 0,
          products: [{ style: "staud/s2601", price: 100, qtyReturned: 2, qtyOrdered: 0 }],
        },
      },
    };
    const { deptVendors } = rollup(
      buildAllRows(scanData, override, { season: "spring26" }),
      scanData,
      override,
      { season: "spring26" }
    );
    const v = vendorRow(deptVendors, "alley", "Staud");
    expect(v.ordered).toBe(300); // LS (5 − 2) × $100, datatail ordered dropped
    expect(v.returned).toBe(200);
  });
...
describe("buildAllRows — live consignment overlay (consignByPid)", () => {
  const scanData = () =>
    makeScanData({
      seasonPids: ["p1"],
      productStats: {
        // Baked scan:data lags the live store (ordered 2 / received 0).
        p1: { qtyOrdered: 2, qtyReceived: 0, retQty: 0, sold: 0, onSale: 0, liveOnHand: 2 },
      },
      pidToType: { p1: "alley" },
      pidToSupplier: { p1: { id: "7", name: "Brand7" } },
      pidToPrice: { p1: 10 },
      pidToCost: { p1: 4 },
      pidToSku: { p1: "b7/s2601" },
    });

  it("uses baked scan:data when no overlay is supplied (backward compatible)", () => {
    const row = buildAllRows(scanData(), null, { season: "spring26" })[0];
    expect(row.lsOrderedQty).toBe(2);
    expect(row.receivedRaw).toBe(0);
  });

  it("supersedes baked ordered/received with the live overlay value", () => {
    const consignByPid = { p1: { qtyOrdered: 4, qtyReceived: 1, qtyReturned: 0 } };
    const row = buildAllRows(scanData(), null, { season: "spring26", consignByPid })[0];
    expect(row.lsOrderedQty).toBe(4);
    expect(row.receivedRaw).toBe(1);
    // Ordered/Received dollars flow from the overlaid quantity × price.
    expect(vendorHeaderTotals([row]).orderedRetail).toBe(40); // 4 × 10
    expect(vendorHeaderTotals([row]).receivedRetail).toBe(10); // (1 − 0) × 10
  });

  it("is authoritative: an LS pid absent from the overlay reads 0 (voided PO)", () => {
    // Overlay is active but has no entry for p1 -> store has nothing for it.
    const consignByPid = { other: { qtyOrdered: 9, qtyReceived: 9, qtyReturned: 0 } };
    const row = buildAllRows(scanData(), null, { season: "spring26", consignByPid })[0];
    expect(row.lsOrderedQty).toBe(0);
    expect(row.receivedRaw).toBe(0);
  });

  it("recomputes returned retail/cost from price/cost when the overlay changes return qty", () => {
    const consignByPid = { p1: { qtyOrdered: 4, qtyReceived: 4, qtyReturned: 3 } };
    const row = buildAllRows(scanData(), null, { season: "spring26", consignByPid })[0];
    expect(row.retQty).toBe(3);
    expect(row.retVal).toBe(30); // 3 × price 10
    expect(row.retCost).toBe(12); // 3 × cost 4
    expect(vendorHeaderTotals([row]).returnedRetail).toBe(30);
  });
});
