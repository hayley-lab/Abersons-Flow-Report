...
//     datatail ordered dollars only for SKUs with no LS PO activity; overlapping
//     SKUs stay LS-only to avoid double-counting. Product-level datatail ordered
//     is net of vendor returns like old flow.sql ((Ordered − Returned) × Price).
//     Vendor-level fallback is already-net when scraped; RMH-backfill gross
//     totals (ordered > orderedScraped) subtract return dollars. Ordered cost
//     follows the same overlap and netting rules; missing costs stay a data gap.
...
import {
  ...
  netOrderedValue,
  netOverrideProductOrderedCost,
  netOverrideProductOrderedRetail,
  netReceivedCost,
  netReceivedRetail,
  netVendorLevelOrdered,
  preferPositive,
  returnedCostValue,
  returnedRetailValue,
  skuMatchesSeason,
...
function overrideExplicitOrderedRetail(op) {
  return num(
    op?.orderedRetail ?? op?.orderedValue ?? op?.orderedAmount ?? op?.ordered ?? op?.orderValue
  );
}

// Merge ordered + returned lines for the same SKU across override records
// (original datatail, rmhret__, rmhord__). MAX qty so shared RMH sources
// are not summed. Price/cost take the first positive value.
function mergeOverrideSku(target, op, rowBySku) {
  ...
  merged.qtyOrdered = Math.max(merged.qtyOrdered, num(op?.qtyOrdered));
  merged.qtyReturned = Math.max(merged.qtyReturned, num(op?.qtyReturned));
  merged.price = preferPositive(merged.price, price);
  merged.cost = preferPositive(merged.cost, cost);
  const explicit = overrideExplicitOrderedRetail(op);
  if (explicit > merged.orderedRetail) merged.orderedRetail = explicit;
  return merged;
}
...
export function rollup(rows, scanData, override, options = {}) {
  ...
  for (const v of Object.values((override && override.vendors) || {})) {
    ...
    const dtBucket = datatailByBucket[k];
    dtBucket.vendorRecords.push({ ordered: num(v.ordered), orderedScraped: v.orderedScraped });
    for (const op of products) {
      const sku = skuOf(op.style);
      dtBucket.bySku[sku] = mergeOverrideSku(dtBucket.bySku[sku], op, rowBySku);
    }
  }

  // Net after merge so rmhret__ qtyReturned lands on the same SKU as ordered.
  for (const dt of Object.values(datatailByBucket)) {
    let bucketReturnedRetail = 0;
    for (const op of Object.values(dt.bySku || {})) {
      const price = preferPositive(op.price, rowBySku.get(skuOf(op.style))?.price);
      const cost = preferPositive(op.cost, rowBySku.get(skuOf(op.style))?.cost);
      const value = netOverrideProductOrderedRetail(op, price);
      const productCost = netOverrideProductOrderedCost(op, cost);
      bucketReturnedRetail += Math.max(0, num(op.qtyReturned) * num(price));
      if (value > 0) dt.hasProductOrdered = true;
      if (productCost > 0) dt.hasProductOrderedCost = true;
      dt.productOrdered += value;
      dt.productOrderedCost += productCost;
      const overlap = hasLsPoActivity(op, productStats, skuToPid, consignByPid);
      dt.overlap = dt.overlap || overlap;
      if (!overlap) {
        dt.uniqueProductOrdered += value;
        dt.uniqueProductOrderedCost += productCost;
      }
    }
    for (const rec of dt.vendorRecords || []) {
      dt.ordered += netVendorLevelOrdered(rec.ordered, rec.orderedScraped, bucketReturnedRetail);
    }
    delete dt.bySku;
    delete dt.vendorRecords;
  }
  ...
  for (const dt of Object.values(datatailByBucket)) {
    ...
    } else if (!dt.overlap && dt.hasProductOrdered) {
      // Product qty is always gross — net at SKU like old flow.sql. Prefer
      // that sum over vendor-level ordered (which may be scrape-net or
      // RMH-backfill gross).
      v.ordered = (v.lsOrdered || 0) + dt.productOrdered;
