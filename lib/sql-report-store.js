...
function summaryData(season, rawData, report, reportTs = null) {
  return {
...
export async function upsertSqlReport(sql, season, rawData, override, options = {}) {
  ...
  const data = summaryData(season, rawData, report, options.reportTs);
...
export async function readSqlReportView({ season, view = "summary", deptId = null }) {
  ...
  if (view === "drows" && deptId != null) {
    const [rows, summary] = await Promise.all([
      readSqlDeptRows(sql, season, deptId),
      readSqlSummary(sql, season),
    ]);
    if (!rows || !summary) return null;
    return { ts: summary.ts, season, deptId: String(deptId), rows };
  }
