...
export function consignmentDate(c) {
  const raw = c?.due_at || c?.received_at || c?.created_at || c?.updated_at || c?.date || c?.delivery_due_at || c?.versioned_at;
  if (!raw) return null;
...
export function productPrice(p) {
  return parseFloat(p?.price_excluding_tax || p?.price || p?.retail_price || p?.unit_price || 0) || 0;
}
...
export function productName(p) {
  const description = p?.description ? String(p.description).replace(/<[^>]*>/g, "").trim() : "";
  return description || p?.name || "";
}

export function productVariant(p) {
  return p?.variant_option_one_value || p?.variant_name || [p?.color, p?.fabric, p?.size].filter(Boolean).join(" / ") || "";
}
...
export function saleContribution(sale, seasonPidSet, pidToPrice = {}) {
  const contribution = { saleId: sale?.id || sale?.sale_id || sale?.invoice_number || null, version: sale?.version || 0, status: sale?.status || "", perPid: {} };
  const saleStatus = String(sale?.status || "").toUpperCase().replace(/[\s,_-]/g, "");
  if (saleStatus === "OPEN" || saleStatus === "PARKED" || saleStatus === "LAYBY" || saleStatus === "LAYAWAY") return contribution;
  ...
  for (const li of (sale?.line_items || [])) {
    ...
    if (!contribution.perPid[pid]) contribution.perPid[pid] = { sold: 0, onSale: 0, saleAmt: 0, soldAmt: 0, returned: 0 };
...
export function derivedOnHand(ps) {
  return Math.max(0, (ps?.qtyReceived || 0) - (ps?.sold || 0) - (ps?.onSale || 0) - (ps?.retQty || 0));
}
