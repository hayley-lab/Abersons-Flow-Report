...
1. **Ordered (cost) data gaps for some datatail rows** — imported/datatail ordered cost now calculates from product-level cost when available, including matched LS row costs. Rows with no reliable product cost remain data gaps; Spring 2026 ordered cost may still be low for those rows. RMH has authoritative `Item.Cost`, so the same local backfill mechanism can close this if needed.
