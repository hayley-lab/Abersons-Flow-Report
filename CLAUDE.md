...
4. **spring25 Ordered — FIXED (Jun 16, 2026); closed-season sold migration gap remains** — the historical datatailor hard-pull had captured only $3.14M of spring25 ordered vs RMH's $4.74M placed-PO truth. **Corrected** via `scripts/backfill-rmh-ordered.mjs` (recomputes each override vendor record's `ordered` from RMH per-SKU placed dollars; now reconciles Δ=$0; reversible via the local backup + `--revert`). See the "spring25 Ordered correction" bullet in the reconciliation section above for mechanics. **Still open:** spring25/fall25 **Sold** is ~10–12% under RMH because not all pre-cutover RMH sales were migrated into LS — an LS data-migration completeness gap (deliberately out of scope: it would need a new rollup path to inject override sold onto LS-matched pids). Affects **closed past seasons only**; going forward everything is LS-native and accurate. fall25 Ordered (−3.4%) left as-is (within tolerance).
