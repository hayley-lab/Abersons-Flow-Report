...
describe("saleContribution — return bucketing via original sale (t4d)", () => {
  ...
  it("falls back to the heuristic when the original is unknown or ambiguous", () => {
    ...
    expect(
      saleContribution(returnSale, seasonPidSet, pidToPrice, { originalPerPid: ambiguous }).perPid
        .p1
...
describe("product-level flow math", () => {
  it("derives on-hand, on-order, and net values with zero floors", () => {
    ...
    expect(netOrderedUnits(10, 2)).toBe(8);
    expect(netOrderedUnits(1, 3)).toBe(0);
  });
});

describe("netOverrideProductOrderedRetail / Cost — old flow.sql netting", () => {
  it("nets vendor-return qty from gross product ordered dollars", () => {
    const op = { qtyOrdered: 5, qtyReturned: 2, price: 100, cost: 40 };
    expect(netOverrideProductOrderedRetail(op, 100)).toBe(300);
    expect(netOverrideProductOrderedCost(op, 40)).toBe(120);
  });

  it("floors at 0 when returns exceed ordered", () => {
    expect(
      netOverrideProductOrderedRetail({ qtyOrdered: 1, qtyReturned: 3, price: 100 }, 100)
    ).toBe(0);
    expect(netOverrideProductOrderedCost({ qtyOrdered: 1, qtyReturned: 3, cost: 40 }, 40)).toBe(0);
  });

  it("nets explicit orderedRetail the same way", () => {
    const op = { orderedRetail: 500, qtyReturned: 2, price: 100 };
    expect(netOverrideProductOrderedRetail(op, 100)).toBe(300);
  });
});

describe("netVendorLevelOrdered — scrape net vs RMH-backfill gross", () => {
  it("leaves already-net scraped ordered alone (no orderedScraped)", () => {
    expect(netVendorLevelOrdered(300, undefined, 200)).toBe(300);
    expect(netVendorLevelOrdered(300, null, 200)).toBe(300);
  });

  it("subtracts returns from RMH-backfill gross (ordered > orderedScraped)", () => {
    expect(netVendorLevelOrdered(500, 300, 200)).toBe(300);
  });

  it("does not subtract when ordered equals orderedScraped", () => {
    expect(netVendorLevelOrdered(300, 300, 200)).toBe(300);
  });
});
...
describe("date and product helpers", () => {
  ...
  // These fixed dates are the regression guard for the UTC construction in
  // seasonSalesFallbackDate: built from a local-time Date, they come out a day
  // early on any machine east of UTC (production runs UTC, so only local/CI
  // timezone differences exposed it).
  it("computes the same season floor east and west of UTC", () => {
    expect(seasonSalesFallbackDate("prespring27")).toBe("2026-02-01");
    expect(seasonSalesFallbackDate("prefall27")).toBe("2026-08-01");
  });
