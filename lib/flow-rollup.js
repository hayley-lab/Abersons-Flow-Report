...
//   - Ordered (retail): datatail POs never made it into LS. The datatail import
//     records ordered as a reliable VENDOR-LEVEL dollar total (per-product
//     ordered qty is sparse/unreliable), so vendor ordered = combine(LS PO
//     ordered, datatail vendor ordered): added when the PO is unique to one
//     source, max when the same PO appears in both. Ordered cost stays LS-only
//     (the old RMH POs carry no LS cost — a data gap, not a bug).
...
function makeRow({
  ...
  return {
    ...
    vendorName: resolved ? supplierName(sup) : "Unassigned",
    sup,
    price: num(price),
    cost: num(cost),
...
function hasLsOverlap(ovProducts, productStats, skuToPid) {
  return (ovProducts || []).some((op) => {
    const pid = skuToPid[skuOf(op.style)];
    const ps = pid != null ? productStats[pid] : null;
    return !!(
      ps &&
...
export function buildAllRows(scanData, override) {
  const data = scanData || {};
  ...
  const season = (scanData && scanData.season) || null;
  ...
  function lsRowFromPid(pid) {
    ...
    return makeRow({
      ...
      price: preferPositive(pidToPrice[pid], overridePriceByPid[pid]),
      cost: preferPositive(pidToCost[pid], overrideCostByPid[pid]),
      lsOrderedQty: stats.qtyOrdered != null ? stats.qtyOrdered : pidToQtyOrdered[pid] || 0,
      receivedRaw: stats.qtyReceived != null ? stats.qtyReceived : pidToQtyReceived[pid] || 0,
      liveOnHand: stats.liveOnHand,
      sold: stats.sold || 0,
      onSale: stats.onSale || 0,
      retQty: stats.retQty != null ? stats.retQty : pidToQtyReturned[pid] || 0,
      retVal: stats.retVal || 0,
      retCost: stats.retCost || 0,
...
export function rollup(rows, scanData, override) {
  ...
  const season = (scanData && scanData.season) || null;
  const datatailByBucket = {}; // deptId|vkey -> { ordered, overlap, id, name, deptId, vkey }
  for (const v of Object.values((override && override.vendors) || {})) {
    if (!v) continue;
    ...
    if (season && !(v.products || []).some((op) => skuMatchesSeason(skuOf(op.style), season)))
      continue;
    const sup = {
      id: v.vendorId || `${v.deptId || v.deptName || "override"}:${v.vendorName}`,
      name: v.vendorName || "Imported Vendor",
    ...
    datatailByBucket[k].ordered += num(v.ordered);
    datatailByBucket[k].overlap =
      datatailByBucket[k].overlap || hasLsOverlap(v.products, productStats, skuToPid);
  }
...
export function rowsForVendor(rows, vendor, dept) {
  return (rows || []).filter((row) => {
    if (!productMatchesVendor(row.sup, vendor)) return false;
    if (!dept) return true;
...
export function vendorRollupTotals(deptVendors, vendor, dept) {
  ...
  const depts = dept ? [String(dept.id)] : Object.keys(deptVendors || {});
  for (const deptId of depts) {
    for (const v of (deptVendors && deptVendors[deptId]) || []) {
      if (!productMatchesVendor({ id: v.id, name: v.name }, vendor)) continue;
