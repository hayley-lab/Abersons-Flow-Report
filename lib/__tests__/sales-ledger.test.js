...
describe("return_for original-sale bucketing (t4d)", () => {
  it("reconcileSale buckets a marker-less return into onSale using the original", async () => {
    const kv = createKv();
    // Original sale sold p1 ON SALE (discounted).
    ...
    const state = { maxVersion: 4, perPid: {}, pidSet: [] };

    // The return line carries NO discount marker and a full-price total, so the
    // heuristic alone would file it as a `sold` return. return_for fixes it.
    await reconcileSale(
      kv,
      "spring26",
      state,
    ...
    expect(state.perPid.p1).toMatchObject({ sold: 0, onSale: -1, returned: 1 });
  });

  it("backfillSales resolves an original from earlier in the same page", async () => {
    const kv = createKv();
    const state = { maxVersion: null, perPid: {}, pidSet: [] };
    ...
    // Net: sold the on-sale unit then returned it → onSale nets to 0, sold stays 0.
    expect(state.perPid.p1).toMatchObject({ sold: 0, onSale: 0, returned: 1 });
  });
});
