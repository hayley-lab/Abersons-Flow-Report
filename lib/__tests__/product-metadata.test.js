...
describe("selectCostBackfillPids", () => {
  ...
  it("skips unpriced pids so we never burn lookups on $0-price rows", () => {
    expect(selectCostBackfillPids(seasonPids, { pidToPrice, costDone: {} }, 10)).not.toContain(
      "p2"
    );
  });
...
describe("shouldFetchHandle", () => {
  ...
  it("skips a SKU whose handle was recorded dead (no in-season LS product)", () => {
    expect(shouldFetchHandle("dead/s261", { skuToPid: {}, deadHandles: { deads261: 1 } })).toBe(
      false
    );
  });
  ...
  it("skips an unmapped SKU when the shared catalog is authoritative (datatail-only)", () => {
    // catalogComplete -> the catalog holds every in-season LS product, so an
    // unmapped override SKU is datatail-only and must not trigger an LS probe.
    expect(
      shouldFetchHandle("ghost/s261", { skuToPid: {}, deadHandles: {}, catalogComplete: true })
    ).toBe(false);
    // Without the catalog flag the same SKU is still probed.
    expect(
      shouldFetchHandle("ghost/s261", { skuToPid: {}, deadHandles: {}, catalogComplete: false })
    ).toBe(true);
  });
});
