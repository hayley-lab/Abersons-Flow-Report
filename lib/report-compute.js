...
// full rows array.
export function computeReport(rawData, override, season, options = {}) {
  const consignByPid = options.consignByPid || null;
  const rows = buildAllRows(rawData, override, { season, consignByPid });
  const { summaryRows, deptVendors } = rollup(rows, rawData, override, { season, consignByPid });
  const health = {
