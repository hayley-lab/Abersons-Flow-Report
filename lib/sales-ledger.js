import { addContribution, returnForId, saleContribution } from "./flow-math";

function ledgerKey(season) {
  return `scan:sales:ledger:${season}`;
}

// Resolve the original sale's per-pid contribution for a customer return so
// saleContribution can bucket the return into the column it originally sold
// from. Checks the in-flight page map first (the original may be earlier in the
// same backfill page, not yet flushed), then the persisted ledger. Returns null
// for non-returns and unknown originals — saleContribution then uses the
// discount heuristic. Only return sales (sale.return_for set) ever read KV.
async function originalPerPidFor(kv, key, sale, pageById) {
  ...
  const orig = parseEntry(await kv.hget(key, idStr));
...
export async function backfillSales(kv, season, state, sales, seasonPidSet, pidToPrice = {}) {
  ...
  for (const sale of sales || []) {
    ...
    const originalPerPid = await originalPerPidFor(kv, ledgerKey(season), sale, writes);
    const contribution = saleContribution(sale, seasonPidSet, pidToPrice, { originalPerPid });
...
export async function reconcileSale(kv, season, state, sale, seasonPidSet, pidToPrice = {}) {
  ...
  const originalPerPid = await originalPerPidFor(kv, key, sale, null);
  const contribution = saleContribution(sale, seasonPidSet, pidToPrice, { originalPerPid });
  contribution.saleId = saleId;
