...
// The cache tag invalidates the read-cache when the underlying data changes:
//   ts       — scan/delta finalize timestamp (new scan/delta)
//   epoch    — datatail import epoch (a re-import)
//   consign  — consignment store version, so newly-entered POs/returns picked up
//              by the request-time consignment overlay invalidate the cache too
//              (the overlay reflects the live store, not the baked scan:data).
export function reportCacheTag(ts, epoch, consign) {
  return `${ts == null ? "none" : ts}:${epoch == null ? 0 : epoch}:${consign == null ? 0 : consign}`;
}
