...
function isVoidedOrDeleted(sale) {
  const status = String(sale?.status || "").toUpperCase().replace(/[\s,_-]/g, "");
...
export async function rebuildSalesState(kv, season) {
  ...
  for (const raw of Object.values(entries)) {
    ...
    if (entry.version && (!rebuilt.maxVersion || entry.version > rebuilt.maxVersion)) rebuilt.maxVersion = entry.version;
  }
