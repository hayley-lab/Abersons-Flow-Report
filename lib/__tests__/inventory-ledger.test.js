...
describe("inventory ledger", () => {
  ...
  it("updates on-hand by per-outlet delta without double-counting re-sent outlets", () => {
    const cache = { version: null, byOutlet: {}, onHand: {} };
    applyInventoryRows(cache, [
      { product_id: "p1", outlet_id: "o1", current_amount: 2 },
      { product_id: "p1", outlet_id: "o2", current_amount: 3 },
    ]);
    expect(liveOnHandFromCache(cache, "p1")).toBe(5);

    // A later page re-sends o1 with a NEW amount; on-hand must reflect the new
    // o1 (4) + unchanged o2 (3) = 7, not 5 + 4.
    applyInventoryRows(cache, [{ product_id: "p1", outlet_id: "o1", current_amount: 4 }]);
    expect(liveOnHandFromCache(cache, "p1")).toBe(7);
    expect(cache.byOutlet).toEqual({ "p1:o1": 4, "p1:o2": 3 });
  });

  it("records touched pids into the provided set", () => {
    const cache = { version: null, byOutlet: {}, onHand: {} };
    ...
    applyInventoryRows(
      ...
      [
        { product_id: "p1", outlet_id: "o1", current_amount: 2 },
        { product_id: "p2", outlet_id: "o1", current_amount: 3 },
      ],
      touched
    );
    expect(Array.from(touched).sort()).toEqual(["p1", "p2"]);
  });
  ...
  it("periodically checkpoints so a mid-loop throw after a checkpoint keeps progress", async () => {
    const kv = createKv();
    ...
    const lsFetch = jest
      ...
      // Second page aborts mid-loop before the post-loop final flush can run.
      .mockRejectedValueOnce(softDeadline);

    // pageSize 1 keeps the loop going (page is "full"); checkpointEvery 1 forces
    // a checkpoint right after page 1, so the abort on page 2 lands after the
    // first page's progress is already durable.
    await expect(
      syncInventoryCache(kv, "__store__", lsFetch, {
        reset: true,
        pageSize: 1,
        checkpointEvery: 1,
      })
    ...
    // The periodic checkpoint persisted page 1 and advanced the cursor, so a
  ...
  it("loses only the uncheckpointed tail when a throw lands before the interval", async () => {
    const kv = createKv();
    const softDeadline = Object.assign(new Error("LS deadline exceeded"), { code: "LS_DEADLINE" });
    const lsFetch = jest
      .fn()
    ...
    // checkpointEvery 5 means page 1 is NOT checkpointed before page 2 throws,
    // so no partial cache is written — acceptable: at most `checkpointEvery`
    // pages are re-paged on resume.
    await expect(
      syncInventoryCache(kv, "__store__", lsFetch, {
        reset: true,
        pageSize: 1,
    ...
    const saved = await loadInventoryCache(kv, "__store__");
    // Only the up-front meta-invalidation write happened; no cache shards yet.
    expect(saved).toEqual({ version: null, byOutlet: {}, onHand: {}, ts: null });
  });
