...
- **Mixed LS/datatail Ordered (retail + cost) (Jun 2026):** vendor ordered dollars are LS PO ordered plus datatail ordered dollars only for override SKUs with no LS PO activity. Overlapping SKUs stay LS-only to avoid double-counting; old override rows without usable per-product ordered dollars fall back to the guarded vendor-level combine. Datatail ordered is net of vendor returns at product qty (`(qtyOrdered − qtyReturned) × price`, matching old `flow.sql`). Vendor-level scrape totals are already net; RMH-backfill gross (`ordered > orderedScraped`) subtracts return dollars. Ordered cost follows the same overlap and netting rules; missing costs stay a data gap instead of being fabricated.
...
- **Ordered (retail)** = `Σ (qtyOrdered − retQty) × price` (old datatailor `flow.sql`: `SUM((FD.Ordered-FD.Returned)*FD.Price)`). LS rows use `netOrderedValue`. Datatail product qty is always gross and is netted the same way; vendor-level fallback is already-net when scraped from datatailor, or gross-minus-returns when RMH backfill set `ordered > orderedScraped`.
- **Ordered (cost)** = LS PO sum(`(qtyOrdered − retQty) × cost`), plus imported/datatail product `(qtyOrdered − qtyReturned) × cost` for non-overlapping override SKUs when product-level cost is available.
- **Received (retail)** = sum(receivedUnits × price), where `receivedUnits = qtyReceived − retQty` (PO `qtyReceived` net of vendor returns; old: `SUM((FD.Received-FD.Returned)*FD.Price)`). **Consignment/migrated fallback:** when `qtyReceived === 0` (consignment or goods migrated with no LS PO record), `receivedUnits` falls back to the live-derived `onHand + sold + onSale`.
...
- **On-hand is NOT a hard-trip field (Aug 2026).** `HARD_QTY_FIELDS` in `lib/report-validate.js` is `qtyOrdered`/`qtyReceived`/`retQty` only — those are re-projected from the same LS consignment cache the rows were built from, so the comparison is deterministic and a single mismatch is a genuine logic bug. `onHand` compares a scan-time snapshot against a live LS fetch hours later, so an item that sells or arrives overnight differs legitimately; as a hard trip it failed Nightly Validate on normal trading (fall26: 2 single-unit moves out of 4011 rows, 0.05%). It is still recorded and still feeds the drifted-product ratio and the season dollar-drift signal, so a SYSTEMATIC on-hand fault still trips drift.
- **The validate budget is anchored at request start (Aug 2026).** `BUDGET_MS` in `pages/api/scan/validate.js` is measured from the top of the handler, not from where the on-hand fetch loop begins. Anchoring it at the loop let slow prep (sharded KV reads + `buildAllRows` + `rollup` on a big override season) stack on top of the full budget and blow past `maxDuration` — spring25 returned HTTP 504 to the nightly Action every night while its data was actually clean. `maxDuration` for the route is 120s, matching the workflow's `curl --max-time 120`.
...
- **Override-sourced fields are skipped, never failed (Jun 2026):** the validator re-derives expected values from Lightspeed alone (live on-hand, LS consignment cache, LS sales aggregate). RMH/datatail overlay values — closed-season vendor returns (`retQty`/`retVal`) and the `spring25`/`fall25` sold/on-sale overlay — never existed in LS, so the LS-only re-derivation legitimately can't reproduce them. `buildAllRows` tags those fields on each row via `row.overrideFields` (set in `lsRowFromPid` when the returns overlay or `shouldUseOverrideSale` wins), and `buildValidationReport` reports them as `SKIP_REASONS.OVERRIDE_SOURCED` (counted in `counts.overrideSourcedFields`) instead of drift. The row's genuinely LS-sourced fields (live on-hand, LS PO ordered/received, LS-native sales) are still checked, so real scan/delta bugs on current seasons still trip. **Without this, the RMH returns + sold overlays show as massive false-positive drift** (e.g. `spring25` 1063 hard-qty "mismatches" + ~62% retail drift) that fails the nightly Action for data that is actually correct.
...
### Full RMH↔report accuracy reconciliation (Jun 16, 2026)
...
- **Ordered $ — reconciled.** Reconciled in **dollars at the `rollup()` level** (LS PO ordered + datatail vendor-level ordered via `combineDollarValue`), per-SKU/crossover-aware — NOT a row-unit sum. **spring25 and fall25 now reconcile to RMH exactly (Δ=$0)** after the Jun 16 corrections: spring25 $4,738,511 and fall25 $2,902,103. **spring26 now reconciles to the deduped LS+RMH union target exactly:** report $4,615,608 = RMH placed $4,597,686 + LS-only ordered $17,922 (`tools/recon-accuracy.js` now checks this target). fall26 is LS-dominated/live; raw RMH comparison remains informational.
- **spring25 Ordered correction (Jun 16, 2026) — DONE.** The datatailor scrape stored **undercounted vendor-total `ordered` dollars** ($3.14M vs RMH's $4.74M placed `POType=0` truth — every RMH SKU was already present, none missing; the scrape's per-product tally marks were near-empty so the report relied on the low scraped vendor totals). Fixed by `scripts/backfill-rmh-ordered.mjs`: it recomputes each existing spring25 override vendor record's top-level `ordered` from RMH per-SKU placed-order dollars (SKU-anchored, each SKU counted once, assigned to the first original datatailor record listing it; RMH SKUs in no original record become tiny `rmhord__` records — $7.7k/5 records here). **Only `ordered` changes** — received/sold/returns and the per-product arrays (cost/price fallback) are untouched, and **no rollup code change was needed** (the rollup already reads the top-level `ordered` for a no-LS season). Reversible: `--write` first dumps the full override to `scripts/out/override-backup-spring25-<ts>.json`, preserves the original in `orderedScraped`, and `--revert <backup>` restores. Diagnostic: `scripts/diff-rmh-ordered.mjs` (read-only gap analysis). The biggest corrections were consignment vendors the scrape badly undercounted (Rene Escobar, Dana Kellin, Judi Powers). Note: this overwrote the opaque legacy figures with RMH placed-PO truth on a closed season (an owner-approved business decision).
- **fall25 Ordered correction (Jun 16, 2026) — DONE.** Final RMH-access verification closed the last soft Ordered gap: report $2,804,633 vs RMH $2,902,103 (−3.4%) became $2,902,103 vs $2,902,103 (Δ=$0). Same reversible script/path as spring25: `node scripts/backfill-rmh-ordered.mjs --seasons fall25 --write`, backup at `scripts/out/override-backup-fall25-2026-06-16T14-54-33-123Z.json`, 188 original records corrected, 5 residual `rmhord__` records ($8,429) written. Only top-level `ordered` changed.
- **spring26 Ordered crossover correction (Jun 16, 2026) — DONE.** Final RMH-access verification found the remaining material Ordered gap was spring26: report $3,865,590 vs RMH placed $4,597,686, while LS PO ordered contributed only $17,922. Fixed with `node scripts/backfill-rmh-ordered.mjs --seasons spring26 --crossover --write`, backup at `scripts/out/override-backup-spring26-2026-06-16T15-11-50-628Z.json`. The crossover mode writes both top-level `ordered` (RMH placed total) and per-product RMH ordered (`qtyOrdered`/`orderedRetail`/`price`) so `rollup()` can keep LS-overlap SKUs LS-only and add RMH-only SKUs exactly once. Dry-run/write corrected 98 original records, refreshed 4,849 products, and wrote 3 residual `rmhord__` records ($15,866). Current recon: spring26 Ordered $ report $4,615,608 = deduped union target $4,615,608 (OK).
...
- **Sold / On-sale — VERIFIED on a net-sales basis (Jun 16).** The earlier spring25 −9.9% / fall25 −11.5% finding compared the report to **gross positive RMH sales** and forgot to net customer returns. RMH `TransactionEntry` truth is now compared as net sold/on-sale (negative return lines stay in the original full-price vs discounted bucket; older snapshots fall back to gross minus customer returns). After writing the closed-season `rmhsold__` overlay, spring25 is +0.7% vs RMH net and fall25 is +1.4% vs RMH net (OK). The overlay is intentionally conservative: RMH replaces LS per pid/sku only when RMH has at least as many net units; otherwise LS wins, so migrated LS sales are never double-counted. spring26/fall26 remain LS-live/crossover seasons, so RMH comparisons are informational.
- **On hand — live from LS** (source of truth; `≠` indicator surfaces divergences). Not a reconciliation target.

**Frozen RMH snapshot:** `scripts/rmh-snapshot.mjs` exports POs (type 0/3 with status), per-SKU sales, and a season summary to `scripts/out/rmh-snapshot-<date>/` (gitignored — supplier names + costs). RMH access ends at handoff, so this is the source-of-truth backup for any later reconciliation/backfill. Final Jun 16 snapshot was copied to both `~/abersons-rmh-snapshots/rmh-snapshot-2026-06-16/` and `~/Creative Cloud Files/abersons-rmh-snapshots/rmh-snapshot-2026-06-16/`.
  - **CAPTURED Jun 15, 2026:** `rmh-snapshot-2026-06-15/` — 155,232 PO SKU rows, 96,156 sales SKU rows. Backed up off-repo to `~/abersons-rmh-snapshots/rmh-snapshot-2026-06-15/` (still get it onto external/cloud storage before machine decommission). Season totals (placed `POType=0` ordered retail/cost; vendor returns; gross sold/on-sale): spring25 $4.74M/$1.99M, 1175 ret, 4040 sold, 951 on-sale; fall25 $2.90M/$1.18M, 167 ret, 3177 sold, 657 on-sale; spring26 $4.60M/$1.59M, 535 ret, 3109 sold, 162 on-sale; fall26 $452k/$186k, 0 ret, 15 sold. (Use the Jun 16 snapshot for explicit net sold/on-sale fields.)
  - **CAPTURED Jun 16, 2026:** `rmh-snapshot-2026-06-16/` — 155,232 PO SKU rows and 96,156 sales SKU rows, plus explicit `soldNet`/`onSaleNet` fields in `summary.json` and `soldNetQty`/`onSaleNetQty` in `sales.tsv`. Net sold+on-sale totals: spring25 4468u, fall25 3348u, spring26 2936u, fall26 15u. Re-captured during final verification before the fall25 Ordered write.
...
4. **spring25/fall25/spring26 Ordered + closed-season Sold — FIXED/RECONCILED (Jun 16, 2026)** — spring25 and fall25 Ordered were corrected via `scripts/backfill-rmh-ordered.mjs` (recomputes each override vendor record's `ordered` from RMH per-SKU placed dollars; both now reconcile Δ=$0; reversible via the local backups + `--revert`). spring26 Ordered was corrected with the script's `--crossover` mode and now reconciles to the deduped LS+RMH union target ($4,615,608). The apparent spring25/fall25 Sold gap was mostly a measurement error: the old harness compared against gross RMH sales instead of netting customer returns. Added `scripts/diff-rmh-sold.mjs`, `scripts/backfill-rmh-sold.mjs`, and `lib/flow-rollup.js` `overrideSoldByProduct`: `rmhsold__` records carry RMH net sold/on-sale buckets and are applied as a **whole-source overlay** per pid/sku (RMH wins only when RMH net units ≥ LS net units; LS wins otherwise). This prevents double-counting migrated LS sales and keeps rare LS-richer products intact. Production KV was written for spring25/fall25 with backups in `scripts/out/override-backup-<season>-2026-06-16T14-*.json`; `backfill-rmh-sold.mjs --revert <backup>` restores. Final recon: Ordered $ Δ=0 for spring25/fall25, spring26 Ordered $ equals the deduped union target, Returned Δ=0 all seasons, closed-season Sold/On-sale OK (spring25 +0.7% vs RMH net, fall25 +1.4% vs RMH net).

### Re-coded SKUs move a product between seasons (Aug 2026)
Staff correct a product's season by editing its SKU season code in Lightspeed (e.g. `neileen3/f26` → `neileen3/ps27` when a Fall 26 buy is really Pre-Spring 27). Two rules make that land correctly:

- **The shared catalog bucket is authoritative for season membership.** `products_seed` reads `scan:catalog:season:{season}` BEFORE restoring prior pids and passes its pid set to `filterRestoredSeasonPids` as `catalogPidSet` (`lib/product-seed.js`). Without this the prior scan's `pidToSku` still holds the OLD SKU, so the SKU gate keeps restoring the pid into its old season on every scan and **the same PO is counted under two seasons**. A pid the catalog has not seen yet (created since the last catalog refresh) is dropped here and re-registered from its PO line item in the consignments phase.
- **The fresher catalog SKU wins over prior `scan:data`.** `seasonProjectionInputs` (`lib/consignment-store.js`, used by `pages/api/scan/consign-cache.js`) unions pid sets from the catalog bucket and prior `scan:data`, but overlays SKUs catalog-LAST across ALL seasons — a per-season merge let one season's stale `scan:data` clobber another season's corrected SKU.

**Latency staff should expect:** a SKU edit only shows up after the store-wide catalog cache refreshes (nightly `/api/scan/catalog`, then re-bucketing) — it is not picked up by the ~10-minute delta sync, which is sales-only. Diagnose with `node scripts/diagnose-po-season.mjs <poNumber> --items`: it prints each line item's live LS SKU, the SKU in the catalog cache, and which season's `scan:data` currently holds the pid ("live ps27 | cached f26 | report: fall26" = edit made, cache not refreshed yet).
