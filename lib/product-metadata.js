...
// The Lightspeed handle for a datatail SKU (the SKU with its slash removed,
// e.g. "cafmrhalo/s2601" → "cafmrhalos2601").
...
// Whether a datatail SKU still needs a handle lookup. Skip it when the SKU is
// already mapped to a pid, or when a prior scan recorded its handle as not
// resolving to an in-season LS product (deadHandles) — those are old RMH SKUs
// that would otherwise be re-fetched on every scan.
//
// When `catalogComplete` is set, the shared store-wide catalog is authoritative:
// it already holds every in-season LS product, so any override SKU not mapped to
// a pid is datatail-only by definition (no LS product to find). Skip the probe
// entirely — the override row is supplied at request time by override-merge.
export function shouldFetchHandle(sku, { skuToPid = {}, deadHandles = {}, catalogComplete = false } = {}) {
  const normalized = String(sku || "")
    .toLowerCase()
    .trim();
  if (!normalized) return false;
  if (skuToPid[normalized]) return false;
  if (catalogComplete) return false;
  return !deadHandles[handleForSku(normalized)];
}
