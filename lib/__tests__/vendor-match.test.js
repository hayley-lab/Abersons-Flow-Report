...
describe("vendorMatchKey", () => {
  it("keys a vendor row in the same space vendorBucketKey uses for sups", () => {
    expect(vendorMatchKey({ id: "2dec4dad", name: "Staud" })).toBe("staud");
    expect(vendorMatchKey({ id: "942", name: "Frank &amp; Eileen" })).toBe("frankeileen");
  });

  it("maps the synthetic Unassigned vendor back to __unassigned__", () => {
    // The bug guard: vendorBucketKey would normalize "Unassigned" to "unassigned",
    // but unresolved-sup rows key to "__unassigned__" — both sides must agree.
    expect(vendorMatchKey({ id: "__unassigned__", name: "Unassigned" })).toBe("__unassigned__");
    expect(vendorBucketKey({ i: "__none__", n: "Unassigned" })).toBe("__unassigned__");
  });

  it("returns null for a missing vendor", () => {
    expect(vendorMatchKey(null)).toBeNull();
  });
});
