...
export function rollup(rows, scanData, override, options = {}) {
  ...
  for (const v of Object.values((override && override.vendors) || {})) {
    ...
    for (const op of products) {
      ...
      recordReturnedRetail += Math.max(0, num(op.qtyReturned) * num(price));
      if (value > 0) bucket.hasProductOrdered = true;
      if (productCost > 0) bucket.hasProductOrderedCost = true;
      bucket.productOrdered += value;
      bucket.productOrderedCost += productCost;
      ...
      if (!overlap) {
        ...
        bucket.uniqueProductOrderedCost += productCost;
      }
    }
    bucket.ordered += netVendorLevelOrdered(v.ordered, v.orderedScraped, recordReturnedRetail);
  }
