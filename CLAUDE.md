...
4. **spring25/fall25 Ordered + closed-season Sold — FIXED/RECONCILED (Jun 16, 2026)** — spring25 and fall25 Ordered were corrected via `scripts/backfill-rmh-ordered.mjs` (recomputes each override vendor record's `ordered` from RMH per-SKU placed dollars; both now reconcile Δ=$0; reversible via the local backups + `--revert`). The apparent spring25/fall25 Sold gap was mostly a measurement error: the old harness compared against gross RMH sales instead of netting customer returns. Added `scripts/diff-rmh-sold.mjs`, `scripts/backfill-rmh-sold.mjs`, and `lib/flow-rollup.js` `overrideSoldByProduct`: `rmhsold__` records carry RMH net sold/on-sale buckets and are applied as a **whole-source overlay** per pid/sku (RMH wins only when RMH net units ≥ LS net units; LS wins otherwise). This prevents double-counting migrated LS sales and keeps rare LS-richer products intact. Production KV was written for spring25/fall25 with backups in `scripts/out/override-backup-<season>-2026-06-16T14-*.json`; `backfill-rmh-sold.mjs --revert <backup>` restores. Final recon: Ordered $ Δ=0 for spring25/fall25, Returned Δ=0 all seasons, closed-season Sold/On-sale OK (spring25 +0.7% vs RMH net, fall25 +1.4% vs RMH net).
