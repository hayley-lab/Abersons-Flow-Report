...
export default function FlowReport() {
  ...
  const setSeasonAndSave = useCallback(function(id) {
    setSeason(id);
    try { localStorage.setItem("flow_season", id); } catch (e) {}
  }, []);
    ...
    try {
      ...
      while (!allDone && !scanAbort.current && iterations < 300) {
        ...
        if (!r.ok) {
          ...
          if (r.status === 503 || r.status === 500) { await new Promise(res => setTimeout(res, 8000)); continue; }
          throw new Error(d.error || "HTTP " + r.status);
        }
            ...
            {!dataLoading && (
              <>
                <KpiRow items={[
                  ...
                  { label: "Received (retail)", value: fmt(totalNetReceived),     sub: totalRecPct.toFixed(1)  + "% of ordered" },
