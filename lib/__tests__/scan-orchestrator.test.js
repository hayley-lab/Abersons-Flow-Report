...
describe("shouldSkipCompletedDrive — one refresh per sync cycle", () => {
  it("skips a complete cache after the cycle refresh has finished", () => {
    expect(
      shouldSkipCompletedDrive({
        kvComplete: true,
  ...
  it("drives a complete cache while this cycle's refresh is pending", () => {
    expect(
      shouldSkipCompletedDrive({
        kvComplete: true,
        resetRequested: false,
...
describe("buildDriverResults — driver response includes every season accurately", () => {
  it("appends skipped seasons after the rich step results", () => {
    const seasonState = [
      { season: "spring26", done: true, action: "advanced" },
      { season: "fall26", done: true, action: "skipped", phase: "done", mode: "full" },
    ];
    ...
    expect(buildDriverResults(seasonState, stepResults)).toEqual([
      ...
      { season: "fall26", action: "skipped", phase: "done", mode: "full" },
    ]);
  });

  it("labels unstepped work pending instead of skipped", () => {
    const seasonState = [
      { season: "spring27", done: false, action: null, phase: "products_seed", mode: "full" },
    ];
    expect(buildDriverResults(seasonState, [])).toEqual([
      {
        season: "spring27",
        action: "pending",
        phase: "products_seed",
  ...
  it("preserves the step .error so the UI can show WHY a season failed (t1b)", () => {
    const seasonState = [
      { season: "spring26", done: true, action: "error" },
      { season: "fall26", done: true, action: "skipped", phase: "done", mode: "full" },
    ];
    const stepResults = [
      { season: "spring26", action: "error", phase: "error", error: "LS 401 auth failed" },
    ];
    const results = buildDriverResults(seasonState, stepResults);
    const spring = results.find((r) => r.season === "spring26");
    expect(spring.error).toBe("LS 401 auth failed");
    // Skipped seasons are appended without clobbering the errored step result.
    expect(results.find((r) => r.season === "fall26")).toEqual({
      season: "fall26",
      action: "skipped",
      phase: "done",
      mode: "full",
