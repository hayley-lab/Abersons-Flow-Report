...
**Frozen RMH snapshot:** `scripts/rmh-snapshot.mjs` exports POs (type 0/3 with status), per-SKU sales, and a season summary to `scripts/out/rmh-snapshot-<date>/` (gitignored — supplier names + costs). RMH access ends at handoff, so this is the source-of-truth backup for any later reconciliation/backfill. Re-run before handoff and copy somewhere safe.
  ...
  - **CAPTURED Jun 16, 2026:** `rmh-snapshot-2026-06-16/` — same row coverage plus explicit `soldNet`/`onSaleNet` fields in `summary.json` and `soldNetQty`/`onSaleNetQty` in `sales.tsv`. Net sold+on-sale totals: spring25 4468u, fall25 3348u, spring26 2936u, fall26 15u.
...
4. **spring25 Ordered + closed-season Sold — FIXED/RECONCILED (Jun 16, 2026)** — spring25 Ordered was corrected via `scripts/backfill-rmh-ordered.mjs` (recomputes each override vendor record's `ordered` from RMH per-SKU placed dollars; now reconciles Δ=$0; reversible via the local backup + `--revert`). The apparent spring25/fall25 Sold gap was mostly a measurement error: the old harness compared against gross RMH sales instead of netting customer returns. Added `scripts/diff-rmh-sold.mjs`, `scripts/backfill-rmh-sold.mjs`, and `lib/flow-rollup.js` `overrideSoldByProduct`: `rmhsold__` records carry RMH net sold/on-sale buckets and are applied as a **whole-source overlay** per pid/sku (RMH wins only when RMH net units ≥ LS net units; LS wins otherwise). This prevents double-counting migrated LS sales and keeps rare LS-richer products intact. Production KV was written for spring25/fall25 with backups in `scripts/out/override-backup-<season>-2026-06-16T14-*.json`; `backfill-rmh-sold.mjs --revert <backup>` restores. Current recon: spring25 +0.7% vs RMH net, fall25 +1.4% vs RMH net (OK). fall25 Ordered (−3.4%) left as-is (within tolerance).
