...
//                                    soldAmt, returned, lastSoldAt } }. The running
//                                    per-PID aggregate, sharded by a stable hash of pid.
...
function applyContributionToAggShards(shards, contribution, sign, shardCount, touched, saleMs = 0) {
  for (const [pid, c] of Object.entries(contribution?.perPid || {})) {
    const index = shardForPid(pid, shardCount);
    const shard = shards[index];
    if (!shard[pid])
      shard[pid] = { sold: 0, onSale: 0, saleAmt: 0, soldAmt: 0, returned: 0, lastSoldAt: 0 };
    const t = shard[pid];
    ...
    if (sign > 0 && saleMs > 0) t.lastSoldAt = Math.max(t.lastSoldAt || 0, saleMs);
    touched.add(index);
  }
}
  ...
  const result = await fetchSalesPages({
    ...
    onPage: async (saleItems) => {
      ...
      for (const sale of saleItems || []) {
        ...
        const saleMs = saleTimestampMs(sale);
        bump(sale?.version || 0);
        if (!saleId) continue;
        ...
        if (backfillMode) {
          ...
          const entry = { perPid: contribution.perPid, version: sale?.version || 0, lastSoldAt: saleMs };
          writes[String(saleId)] = entry;
          applyContributionToAggShards(shards, entry, 1, effectiveShardCount, touched, saleMs);
        } else {
          ...
          const entry = { perPid: contribution.perPid, version, lastSoldAt: saleMs };
