...
describe("buildValidationReport — hard quantity mismatches", () => {
  it("records a fresh-LS on-hand mismatch without making it a hard trip", () => {
    const report = buildValidationReport({
      season: "spring26",
      rows: [cleanRow()],
    ...
    expect(report.drift.reasons.map((r) => r.code)).not.toContain("hard-qty-mismatch");
    // Still a drift signal via the ratio threshold (1 of 1 checked).
    expect(report.counts.driftedProducts).toBe(1);
    expect(report.drift.reasons.map((r) => r.code)).toContain("drifted-products");
  });

  // Regression: on-hand is a scan-time snapshot compared against a live LS read
  // hours later, so a couple of overnight sales/receipts are normal. As a hard
  // trip these failed Nightly Validate on real trading (fall26, 2/4011 rows).
  it("does not trip drift when a few sampled rows moved on hand overnight", () => {
    const rows = [];
    const freshOnHand = {};
    const freshConsign = {};
    const freshSales = {};
    for (let i = 0; i < 500; i += 1) {
      const pid = `p${i}`;
      rows.push(cleanRow({ pid }));
      // Two products traded between the scan and this validation run.
      freshOnHand[pid] = i < 2 ? 2 : 3;
      freshConsign[pid] = { qtyOrdered: 5, qtyReceived: 5, qtyReturned: 0 };
      freshSales[pid] = { sold: 2, onSale: 0, saleAmt: 0 };
    }
    const report = buildValidationReport({
      season: "fall26",
      rows,
    ...
    expect(report.counts.byField.onHand).toBe(2);
    expect(report.counts.hardQtyMismatches).toBe(0);
    expect(report.drift.tripped).toBe(false);
  });

  it("still trips drift when on-hand is systematically wrong", () => {
    const rows = [];
    const freshOnHand = {};
    const freshConsign = {};
    const freshSales = {};
...
describe("buildValidationReport — override-sourced fields are skipped, not failed", () => {
  // RMH/datatail overlay data (closed-season vendor returns and sold/on-sale)
  // never existed in Lightspeed, so the LS-only re-derivation cannot reproduce
  // it. buildAllRows tags those fields on the row; the harness must skip them.
  it("skips an override-sourced vendor return instead of a hard mismatch", () => {
    const report = buildValidationReport({
      season: "spring25",
      rows: [cleanRow({ retQty: 5, overrideFields: ["retQty"] })],
      freshOnHand: { p1: 3 },
      // LS consignment cache has no return for this RMH-only pid.
      freshConsign: { p1: { qtyOrdered: 5, qtyReceived: 5, qtyReturned: 0 } },
      freshSales: { p1: { sold: 2, onSale: 0, saleAmt: 0 } },
    });
    expect(report.mismatches.find((m) => m.field === "retQty")).toBeUndefined();
    expect(report.counts.hardQtyMismatches).toBe(0);
    expect(report.counts.driftedProducts).toBe(0);
    expect(report.counts.overrideSourcedFields).toBe(1);
    expect(
      report.skipped.some((s) => s.field === "retQty" && s.reason === SKIP_REASONS.OVERRIDE_SOURCED)
    ).toBe(true);
    expect(report.drift.tripped).toBe(false);
  });

  it("skips override-sourced sold/onSale/saleAmt and does not add retail drift", () => {
    const report = buildValidationReport({
      season: "fall25",
      rows: [
        cleanRow({
          ...
          saleAmt: 120,
          price: 100,
          overrideFields: ["sold", "onSale", "saleAmt"],
        }),
      ],
      freshOnHand: { p1: 3 },
      freshConsign: { p1: { qtyOrdered: 5, qtyReceived: 5, qtyReturned: 0 } },
      // LS sales aggregate disagrees with the RMH overlay — must be ignored.
      freshSales: { p1: { sold: 1, onSale: 0, saleAmt: 0 } },
    });
    expect(report.mismatches).toHaveLength(0);
    expect(report.counts.driftedProducts).toBe(0);
    expect(report.counts.seasonRetailDrift).toBe(0);
    expect(report.counts.overrideSourcedFields).toBe(3);
    expect(report.drift.tripped).toBe(false);
  });

  it("still verifies non-override fields on the same row", () => {
    // retQty is override-sourced (skipped) but on-hand is LS-live and drifts.
    const report = buildValidationReport({
      season: "spring25",
      rows: [cleanRow({ retQty: 5, overrideFields: ["retQty"] })],
      freshOnHand: { p1: 1 }, // report says 3 -> real LS on-hand drift
      freshConsign: { p1: { qtyOrdered: 5, qtyReceived: 5, qtyReturned: 0 } },
      freshSales: { p1: { sold: 2, onSale: 0, saleAmt: 0 } },
    });
    expect(report.mismatches.find((m) => m.field === "retQty")).toBeUndefined();
    expect(report.mismatches.find((m) => m.field === "onHand")).toMatchObject({ actual: 1 });
    expect(report.counts.driftedProducts).toBe(1);
    expect(report.drift.tripped).toBe(true);
  });
});
...
describe("buildValidationReport — rollup and data-gap checks", () => {
  ...
  it("reports coverage percentages and known data gaps", () => {
    ...
    // The Staud spring26 operational checklist was removed once the return was
    // verified in RMH (correctly spring26) and backfilled into the report.
    expect(report.dataGaps.map((g) => g.code)).not.toContain("staud-spring26-checklist");
  });
});
...
describe("evaluateDrift — season retail-$ threshold", () => {
  it("trips when season retail drift exceeds 0.5% even without a hard mismatch", () => {
    ...
    // and the single drifted product is high-priced so the dollar signal is the
    // only threshold that can trip. (On-hand is not a hard field — see
    // HARD_QTY_FIELDS — so nothing else can carry this assertion.)
    const rows = [];
    for (let i = 0; i < 500; i++) {
      rows.push(
        cleanRow({
    ...
    expect(report.counts.hardQtyMismatches).toBe(0);
    // 1 of 500 products drifted = 0.2%, under the 0.5% drifted-product threshold.
    expect(report.counts.driftedProducts).toBe(1);
    expect(report.counts.seasonRetailDrift).toBeCloseTo(1000000, 5); // 10 units * $100k
    expect(report.counts.seasonRetailDriftRatio).toBeGreaterThan(
      DEFAULT_THRESHOLDS.seasonDollarPct
    );
    const drift = evaluateDrift(report, { ...DEFAULT_THRESHOLDS });
    expect(drift.tripped).toBe(true);
    expect(drift.reasons.map((r) => r.code)).toContain("season-retail-drift");
  });
