...
describe("shouldFullCatalogScan", () => {
  ...
  it("skips the per-season scan when seeded from the shared catalog, even if empty", () => {
    // A future season with no products: the shared catalog bucketed it (empty),
    // so the catalog is authoritative and the per-season /search must not run.
    expect(
      shouldFullCatalogScan({
        searchEnabled: true,
        fullRebuild: true,
