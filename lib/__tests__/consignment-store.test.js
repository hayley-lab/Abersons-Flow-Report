...
describe("seasonConsignmentBuckets", () => {
  ...
  it("uses known SKUs to block wrong-season dated projection", () => {
    const buckets = seasonConsignmentBuckets(
      {
        c1: {
          ...
          type: "SUPPLIER",
          date: "2026-05-01",
          perPid: { pFall: { qtyOrdered: 7, qtyReceived: 4, qtyReturned: 0 } },
        },
      },
      {
        seasons: ["prespring27"],
        ...
        pidToSku: { pFall: "vendor/f2601" },
      }
    );

    expect(buckets.prespring27.pidToQtyOrdered).toEqual({});
    expect(buckets.prespring27.pidToQtyReceived).toEqual({});
  });

  it("projects broadened prior pids when their SKU matches the season", () => {
    const buckets = seasonConsignmentBuckets(
      {
        c1: {
          id: "c1",
          type: "SUPPLIER",
          date: "2026-01-15",
          perPid: { pArchived: { qtyOrdered: 3, qtyReceived: 2, qtyReturned: 0 } },
        },
      },
      {
        seasons: ["spring26"],
        seasonPidSets: { spring26: new Set(["pArchived"]) },
        scanRanges: { spring26: seasonScanDateRange("spring26") },
        pidToSku: { pArchived: "vendor/s2601" },
      }
    );

    expect(buckets.spring26.pidToQtyOrdered).toEqual({ pArchived: 3 });
    expect(buckets.spring26.pidToQtyReceived).toEqual({ pArchived: 2 });
  });

  it("keeps the pid-set fallback when SKU is unknown", () => {
    const buckets = seasonConsignmentBuckets(
      {
        c1: {
          id: "c1",
          type: "SUPPLIER",
          date: "2026-01-15",
          perPid: { pUnknown: { qtyOrdered: 4, qtyReceived: 4, qtyReturned: 0 } },
        },
      },
      {
        seasons: ["spring26"],
        seasonPidSets: { spring26: new Set(["pUnknown"]) },
        scanRanges: { spring26: seasonScanDateRange("spring26") },
        pidToSku: {},
      }
    );

    expect(buckets.spring26.pidToQtyOrdered).toEqual({ pUnknown: 4 });
    expect(buckets.spring26.pidToQtyReceived).toEqual({ pUnknown: 4 });
  });

  it("attributes an undated consignment only to the pid's unique owner season", () => {
    // Undated entry: each pid lands in the ONE season whose SKU set owns it,
    // never projected into every season (regression for the inflation bug).
    const undatedEntries = {
      u1: {
        ...
        date: null,
        perPid: {
          p1: { qtyOrdered: 4, qtyReceived: 2, qtyReturned: 0 },
          p2: { qtyOrdered: 6, qtyReceived: 6, qtyReturned: 0 },
        },
      },
    };
    const buckets = seasonConsignmentBuckets(undatedEntries, {
      seasons: ["spring26", "fall26"],
      seasonPidSets: { spring26: new Set(["p1"]), fall26: new Set(["p2"]) },
      scanRanges: {
        spring26: seasonScanDateRange("spring26"),
        fall26: seasonScanDateRange("fall26"),
      },
    });
    expect(buckets.spring26.pidToQtyOrdered).toEqual({ p1: 4 });
    expect(buckets.fall26.pidToQtyOrdered).toEqual({ p2: 6 });
    // p1 must NOT leak into fall26 and p2 must NOT leak into spring26.
    expect(buckets.fall26.pidToQtyOrdered.p1).toBeUndefined();
    expect(buckets.spring26.pidToQtyOrdered.p2).toBeUndefined();
  });

  it("uses known SKUs to choose the right owner for undated shared pids", () => {
    const undatedEntries = {
      u1: {
        id: "u1",
        type: "SUPPLIER",
        date: null,
        perPid: { pShared: { qtyOrdered: 5, qtyReceived: 5, qtyReturned: 0 } },
      },
    };
    const buckets = seasonConsignmentBuckets(undatedEntries, {
      seasons: ["spring26", "fall26"],
      seasonPidSets: {
        spring26: new Set(["pShared"]),
        fall26: new Set(["pShared"]),
      },
      scanRanges: {
        spring26: seasonScanDateRange("spring26"),
        fall26: seasonScanDateRange("fall26"),
      },
      pidToSku: { pShared: "vendor/s2601" },
    });

    expect(buckets.spring26.pidToQtyOrdered).toEqual({ pShared: 5 });
    expect(buckets.fall26.pidToQtyOrdered).toEqual({});
  });

  it("excludes an undated consignment pid shared by more than one season", () => {
    const undatedEntries = {
      u2: {
        id: "u2",
        type: "SUPPLIER",
        date: null,
        perPid: { pShared: { qtyOrdered: 10, qtyReceived: 8, qtyReturned: 0 } },
      },
    };
    const buckets = seasonConsignmentBuckets(undatedEntries, {
      seasons: ["spring26", "fall26"],
    ...
    // Ambiguous pid is dropped from BOTH seasons instead of inflating each.
    expect(buckets.spring26.pidToQtyOrdered).toEqual({});
    expect(buckets.fall26.pidToQtyOrdered).toEqual({});
  });
});
...
describe("seasonProjectionInputs", () => {
  it("unions pid sets from the catalog bucket and prior scan data", () => {
    const { seasonPidSets } = seasonProjectionInputs(["fall26"], {
      catalogBuckets: [{ seasonPids: ["pCatalog"] }],
      priorData: [{ seasonPids: ["pPrior"] }],
    });

    expect([...seasonPidSets.fall26].sort()).toEqual(["pCatalog", "pPrior"]);
  });

  // Regression: Frank & Eileen PO 85626 (Aug 2026) was re-coded "/f26" -> "/ps27".
  // fall26's stale scan:data SKU must not outrank prespring27's fresh catalog SKU,
  // or the PO projects into both seasons.
  it("lets a later season's stale scan:data SKU not clobber a fresh catalog SKU", () => {
    const { pidToSku } = seasonProjectionInputs(["prespring27", "fall26"], {
      catalogBuckets: [
        { seasonPids: ["p1"], pidToSku: { p1: "neileen3/ps27" } },
        { seasonPids: [] },
      ],
      priorData: [null, { seasonPids: ["p1"], pidToSku: { p1: "neileen3/f26" } }],
    });

    expect(pidToSku.p1).toBe("neileen3/ps27");
  });

  it("falls back to the prior scan SKU for pids no catalog bucket lists", () => {
    const { pidToSku } = seasonProjectionInputs(["fall26"], {
      catalogBuckets: [{ seasonPids: [] }],
      priorData: [{ seasonPids: ["pArchived"], pidToSku: { pArchived: "nperry/f26" } }],
    });

    expect(pidToSku.pArchived).toBe("nperry/f26");
  });

  it("derives each season's scan date range", () => {
    const { scanRanges } = seasonProjectionInputs(["prespring27"], {});
    expect(scanRanges.prespring27).toEqual(seasonScanDateRange("prespring27"));
  });
});
