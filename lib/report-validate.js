...
import { rowsForVendor, vendorHeaderTotals, vendorRollupTotals } from "./flow-rollup";
...
function dataGapSummary(rows = [], season = "") {
  const gaps = [];
  ...
  if (season === "spring26") {
    gaps.push({
      code: "staud-spring26-checklist",
      vendorName: "Staud",
      detail: "Operational check: confirm Carrie's Staud vendor return was entered in spring26.",
    });
  }
  ...
  return {
    counts: {
      orderedCostGap,
      zeroPriceGap,
      operationalChecklist: season === "spring26" ? 1 : 0,
      total: gaps.length,
    },
    gaps,
  };
...
export function buildValidationReport({
  ...
  if (rollupResult && rollupResult.deptVendors && rollupResult.summaryRows) {
    const vendorFields = [
      ["rollupReceived", "receivedRetail", "received"],
      ["rollupCost", "receivedCost", "receivedCost"],
      ["rollupReturned", "returnedRetail", "returned"],
      ["rollupReturnedCost", "returnedCost", "returnedCost"],
      ["rollupSold", "soldRetail", "sold"],
      ["rollupOrderedCost", "orderedCost", "orderedCost"],
    ];
    for (const [deptId, vendors] of Object.entries(rollupResult.deptVendors || {})) {
      for (const v of vendors || []) {
        const dept = { id: deptId };
        const rowSum = vendorHeaderTotals(rowsForVendor(rows, v, dept));
        const header = vendorRollupTotals(rollupResult.deptVendors, v, dept);
  ...
  const gaps = dataGapSummary(rows, season);
