...
describe("syncSalesStore cold build", () => {
  ...
  it("records lastSoldAt on aggregate rows and ledger entries", async () => {
    const kv = createKv();
    const saleDate = "2025-03-04T12:34:56.000Z";

    await syncSalesStore(kv, onceThenEmpty([sale("s1", 5, [li("p1", 1, 100)], { sale_date: saleDate })], 5), {
      reset: true,
      dateFrom: "2025-01-01",
    });
    ...
    const agg = await loadSalesAgg(kv);
    expect(agg.p1.lastSoldAt).toBe(saleMs);
    expect(kv.hashes.get(SALES_STORE_LEDGER_KEY).get("s1")).toMatchObject({
      lastSoldAt: saleMs,
    });
  });
...
describe("syncSalesStore incremental", () => {
  ...
  it("keeps lastSoldAt as a forward-only max across edits and voids", async () => {
    const kv = createKv();
    const originalDate = "2025-06-01T00:00:00.000Z";
    const earlierEditDate = "2025-05-01T00:00:00.000Z";
    const laterDate = "2025-07-01T00:00:00.000Z";

    await syncSalesStore(kv, onceThenEmpty([sale("s1", 5, [li("p1", 1, 100)], { sale_date: originalDate })], 5), {
      reset: true,
      dateFrom: "2025-01-01",
    });

    const edit = jest.fn(async (path) => {
      if (path.includes("after=5")) {
        return {
          version: { max: 8 },
          data: [sale("s1", 8, [li("p1", 2, 200)], { sale_date: earlierEditDate })],
        };
      }
      return { data: [] };
    });
    await syncSalesStore(kv, edit, { dateFrom: "" });

    let agg = await loadSalesAgg(kv);
    expect(agg.p1.sold).toBe(2);
    expect(agg.p1.lastSoldAt).toBe(Date.parse(originalDate));

    const laterSale = jest.fn(async (path) => {
      if (path.includes("after=8")) {
        return {
          version: { max: 10 },
          data: [sale("s2", 10, [li("p1", 1, 100)], { sale_date: laterDate })],
        };
      }
      return { data: [] };
    });
    await syncSalesStore(kv, laterSale, { dateFrom: "" });

    agg = await loadSalesAgg(kv);
    expect(agg.p1.sold).toBe(3);
    expect(agg.p1.lastSoldAt).toBe(Date.parse(laterDate));

    const voidLaterSale = jest.fn(async (path) => {
      if (path.includes("after=10")) {
        return {
          version: { max: 11 },
          data: [sale("s2", 11, [li("p1", 1, 100)], { sale_date: laterDate, status: "VOIDED" })],
        };
      }
      return { data: [] };
    });
    await syncSalesStore(kv, voidLaterSale, { dateFrom: "" });
