...
export function netOrderedUnits(qtyOrdered, retQty) {
  return Math.max(0, (Number(qtyOrdered) || 0) - (Number(retQty) || 0));
}

export function netOrderedValue(ps, price) {
  return Math.max(0, netOrderedUnits(ps?.qtyOrdered, ps?.retQty) * (price || 0));
}

function overrideNum(x) {
  const n = typeof x === "number" ? x : parseFloat(x);
  return Number.isFinite(n) ? n : 0;
}

function overrideGrossOrderedRetail(op, price) {
  const explicit = overrideNum(
    op?.orderedRetail ?? op?.orderedValue ?? op?.orderedAmount ?? op?.ordered ?? op?.orderValue
  );
  if (explicit > 0) return explicit;
  const p = price > 0 ? price : overrideNum(op?.price);
  return Math.max(0, overrideNum(op?.qtyOrdered) * p);
}

// Datatail/RMH product qty is always gross. Net like old flow.sql:
// (Ordered − Returned) × Price, floored at 0.
export function netOverrideProductOrderedRetail(op, price) {
  const p = price > 0 ? price : overrideNum(op?.price);
  const gross = overrideGrossOrderedRetail(op, p);
  return Math.max(0, gross - overrideNum(op?.qtyReturned) * p);
}

export function netOverrideProductOrderedCost(op, cost) {
  const c = cost > 0 ? cost : overrideNum(op?.cost);
  let qty = overrideNum(op?.qtyOrdered);
  if (qty <= 0) {
    const p = overrideNum(op?.price);
    const gross = overrideGrossOrderedRetail(op, p);
    if (p > 0 && gross > 0) qty = gross / p;
  }
  return Math.max(0, netOrderedUnits(qty, op?.qtyReturned) * c);
}

// Vendor-level override `ordered` is already net when scraped from datatailor
// (DollarsOrdered). RMH ordered backfill overwrites it with gross POType=0 and
// preserves the scrape in `orderedScraped`. Only subtract returns from the
// gross backfill shape — subtracting from an already-net scrape understates.
export function netVendorLevelOrdered(ordered, orderedScraped, returnedRetail) {
  const o = overrideNum(ordered);
  const ret = Math.max(0, overrideNum(returnedRetail));
  if (orderedScraped == null || orderedScraped === "") return Math.max(0, o);
  const scraped = typeof orderedScraped === "number" ? orderedScraped : parseFloat(orderedScraped);
  if (!Number.isFinite(scraped)) return Math.max(0, o);
  if (o > scraped) return Math.max(0, o - ret);
  return Math.max(0, o);
}
