...
//   - Ordered (retail): datatail POs never made it into LS. For mixed vendors,
//     add datatail ordered dollars only for SKUs with no LS PO/return activity;
//     overlapping SKUs stay LS-only to avoid double-counting. If an old override
//     lacks usable per-product dollars, fall back to the guarded vendor-level
//     combine rule. Ordered cost stays LS-only (old RMH POs carry no LS cost).
...
function hasLsPoOrReturnActivity(op, productStats, skuToPid) {
  const pid = skuToPid[skuOf(op?.style)];
  const ps = pid != null ? productStats[pid] : null;
  return !!(ps && ((ps.qtyOrdered || 0) > 0 || (ps.qtyReceived || 0) > 0 || (ps.retQty || 0) > 0));
}
...
export function rollup(rows, scanData, override, options = {}) {
  ...
  for (const v of Object.values((override && override.vendors) || {})) {
    ...
    for (const op of products) {
      ...
      const overlap = hasLsPoOrReturnActivity(op, productStats, skuToPid);
      bucket.overlap = bucket.overlap || overlap;
      if (!overlap) bucket.uniqueProductOrdered += value;
    }
  }
  ...
  for (const dt of Object.values(datatailByBucket)) {
    ...
    if (dt.overlap && dt.hasProductOrdered) {
      v.ordered = v.lsOrdered + dt.uniqueProductOrdered;
    } else {
